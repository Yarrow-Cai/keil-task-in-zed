...
IF /I "%TASK%"=="open"    SET OPEN_MODE=1
IF not defined ARGS IF not defined OPEN_MODE (
    echo [ERROR] 无效指令 '%TASK%'. 请使用 'build', 'rebuild', 'flash' 或 'open'.
    exit /b 1
)
echo [INFO] 正在执行: %TASK%...
echo ----------------------------------------------------

::: open 模式: 用 Keil GUI 打开工程, 不等待、不产生编译日志
if defined OPEN_MODE (
    start "" "%KEIL_PATH%" "%PROJECT_FILE%"
    echo [SUCCESS] 已在 Keil 中打开工程: %PROJECT_FILE%
    endlocal
    exit /b 0
)
