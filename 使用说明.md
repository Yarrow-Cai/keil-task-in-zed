...
如果你使用 Claude Code，只需在本仓库中让 AI 帮你完成全部配置：

1.  在本仓库中打开终端或 Claude Code 会话
2.  输入：**"为我的 Keil 项目配置 .zed，路径是 `D:\MyProject`"**
3.  AI 会自动：
    *   从 `configure-keil-zed` Skill 自带模板复制 `.zed/` 文件夹到目标项目
    *   自动搜索 Keil 安装路径或提示你提供
    *   更新 `keil_task.bat` 中的 `KEIL_PATH`
    *   打印部署摘要和使用说明
...
#### 打开工程 (Keil GUI)
运行 `Keil: 打开工程 (Keil GUI)` 任务会：
1. 自动查找 `.uvprojx` 或 `.uvproj` 工程文件
2. 调用 Keil 的 `start` 异步启动，直接以 GUI 模式打开该工程
3. 不产生编译日志，不阻塞 Zed 终端

适用于需要硬件调试、断点设置、外设配置界面等必须在 Keil 内完成的场景。
...
*   `.claude/skills/configure-keil-zed/SKILL.md`: AI 一键配置 Skill。Skill 自带完整 `.zed/` 模板（含脚本与 `keil2clangd.exe`），当你在任意目录中让 AI 帮你部署 `.zed/` 到目标项目时，AI 会加载此 Skill，从 Skill 目录复制模板并自动完成路径配置。
...
如果你在多个项目中使用此工具集，有两种方式：

**方式一: 使用 AI Skill（推荐）**

在本仓库中直接告诉 AI：
> "为 `D:\ProjectA` 和 `D:\ProjectB` 配置 .zed"

AI 会依次为每个项目完成复制和路径配置，全程无需手动操作。
