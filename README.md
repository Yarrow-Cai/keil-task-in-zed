...
### 功能说明
本项目新增了 clangd 自动配置功能，可以为 Zed 编辑器自动生成 clangd 所需的配置文件，提升 C/C++ 代码的智能提示和语法检查功能。

### 使用方法
1. 在 Zed 编辑器中按下 `Ctrl+Shift+P` 打开命令面板。
2. 输入 `Tasks: Run` 并选择。
3. 选择 `Keil: clangd 自动配置` 任务。

### 生成的配置文件
脚本执行后会生成以下两个配置文件：
- `.clangd`: 包含 clangd 的配置信息，已经针对 Keil 项目进行了优化。
- `compile_commands.json`: 编译命令数据库模板文件，需要根据实际项目配置更新。

### 优化建议
- 如果您使用 Keil MDK 5.30+ 版本，可以通过 Keil 的设置生成真实的编译命令文件。
- 更多 clangd 配置选项可以参考官方文档：https://clangd.llvm.org/config
- 您可以根据项目需求修改 `.clangd` 文件中的配置参数。
